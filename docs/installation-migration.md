...
Also, in version 2 only one instance of HAppy Irrigation is allowed. In V1, if you wanted multiple zones you needed to install multiple instances of HAppy Irrigation.
...
2. In Home Assistant, go to Settings>Devices&Services>Integrations>HAppy Irrigation, or use [this link](https://my.home-assistant.io/redirect/integration/?domain=happy_irrigation).
3. For each instance of HAppy Irrigation that you have configured, click the 'three vertical dots' menu and select 'download diagnostics':
...
```
{
  ...
  "data": {
    "config": {
      ...
      "source": "user",
      "unique_id": "HAppy Irrigation",
      "disabled_by": null
    }
  }
}
```
6. After you have downloaded and confirmed all diagnostics files (one for each instance of HAppy Irrigation), remove all instances of HAppy Irrigation.
7. Restart Home Assistant
8. Use HACS to install HAppy Irrigation V202X.X. Make sure you use the correct API key and API version if you want to continue using Open Weather Map (recommended). Also, refer to your diagnostics file to remind yourself if and which sensors you used to supply part of the required weather data.
