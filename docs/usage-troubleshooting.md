...
- the diagnostic file / storage file. You can either generate a diagnostic file or get the `happy_irrigation.storage` file from the `configuration/.storage` folder. To download a diagnostics file, in Home Assistant, go to Settings >Devices&Services > Integrations >HAppy Irrigation, or use this [link](https://my.home-assistant.io/redirect/integration/?domain=happy_irrigation). Click the 'three vertical dots' menu and select 'download diagnostics'.
